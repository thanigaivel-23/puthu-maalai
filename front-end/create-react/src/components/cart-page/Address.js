...
const Address = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setCity(data.results !== 0 ? data.results[postalCode][0].province : '')
            // console.log(data.results[postalCode][0].state);
        }
        ...
        <>
            ...
            <form onSubmit={submitHandler} className='md:border-2 md:w-[70%] lg:w-[50%] md:mx-auto md:my-7'>
                ...
                <div>
                    ...
                    <input type="text" readOnly placeholder='City / District' value={city} onChange={(e) => setCity(e.target.value)} required className='outline-none border rounded-sm box-border w-11/12 m-3 px-3 py-2 focus:border-gray-700 placeholder:text-sm' />
                    <input type="text" readOnly placeholder='State' value={state} onChange={(e) => setState(e.target.value)} required className='outline-none border rounded-sm box-border w-11/12 m-3 px-3 py-2 focus:border-gray-700 placeholder:text-sm' />
                    <input type="number" placeholder='Pin Code' value={postalCode} onTouchEnd={() => { console.log(23); }} onChange={(e) => setPostalCode(e.target.value)} required className='outline-none border rounded-sm box-border w-11/12 m-3 px-3 py-2 focus:border-gray-700 placeholder:text-sm' />
                </div>
