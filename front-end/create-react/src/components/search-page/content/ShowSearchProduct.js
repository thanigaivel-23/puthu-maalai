...
const ShowSearchProduct = ({products,loading}) => {
                                    ...
                                    {products && products.map(product => (
                                          ...
                                          <div className="  flex  flex-col relative border-2" key={product._id}>
                                            ...
                                            <Link to={`/product/${product._id}`} preventScrollReset={true}>
                                              <img src={product.images.length > 0 ?product.images[0].image : null} alt="" className="h-36 xs:h-56 lg:h-72 w-full object-cover " />
                                            </Link>
                              
                                            {/* product details */}
                                            <div className="px-1 md:px-3  pt-1 md:pt-3 md:pb-3 pb-2">
                                              <Link to={`/product/${product._id}`}>
                                                <h3 className="font-bold h-10 md:h-[58px] overflow-hidden text-sm md:text-xl">{product.name}</h3>
                                              </Link>

                                              <p className="h-8 md:h-[54px] overflow-hidden text-xs md:text-base text-gray-500">
                                                {product.description}
                                              </p>
                              
                                              {/* ratings */}
                                              <div className=" md:flex justify-between mt-1 text-xs md:text-base">
                                                <div className="flex items-center">
                                                  <FaStar className="text-rose-600 mr-2" /> {product.ratings} ({product.numOfReviews} Reviews)
                                                </div>
                                                <div className="text-green-500">{product.stock?<span>In Stock</span>:<span>Out of Stock</span>}</div>
                                              </div>
