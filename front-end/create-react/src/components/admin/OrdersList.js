...
import { adminOrders, deleteOrders } from '../../actions/orderActions'
import { clearOrderDeleted, clearOrderError } from '../../slices/orderSlice'

const OrdersList = () => {
  const dispatch = useDispatch()
  const { adminOrdersList = [], loading = true, error, isOrderDeleted } = useSelector(state => state.orderState)
  ...
  const handleFilter = (e) => {
    ...
    if (e.target.value === '') {
      ...
      if (filterResult.length > 0) {
        ...
        setOrderItems([{ '_id': "No Data", 'price': "No Data", 'totalPrice': "No Data", 'action': "No Data" }])
      }
    }
            ...
            <tbody>
              {orderItems.length > 0 && orderItems.map((order, index) => (
                <tr className='odd:bg-gray-100 text-sm md:text-base w-[100%] lg:w-[70%]' key={index}>
                  ...
                  <td className='border-x border-[#ddd] text-center p-2 xs:p-3' > {order.orderItems && order.orderItems.length}</td>
                  <td className='border-x border-[#ddd] text-center p-2 xs:p-3' >&#8377; {order.totalPrice}</td>
                  <td className='border-x border-[#ddd] text-center p-2 xs:p-3' >{order.orderStatus}</td>
                  <td className='border-x  border-[#ddd] text-center  p-2 xs:p-3 flex justify-center gap-x-2' >
                    <Link to={`/admin/order/update/${order._id}`} className='bg-blue-500 p-2 xs:p-3 rounded-md '><MdOutlineEdit className='text-white text-xl' /></Link>
